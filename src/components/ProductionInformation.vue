    ...
    <section class="py-12 md:py-16">
      <div class="container mx-auto px-4">
        ...
        <div class="grid lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-8">
            <div class="rounded-xl overflow-hidden shadow-sm">
              <img :src="mainImage" alt="Project image"
                   class="w-full h-[320px] sm:h-[420px] lg:h-[500px] object-cover" />
            </div>
          </div>
          ...
          <aside class="lg:col-span-4">
            <div class="bg-gray-100 rounded-xl overflow-hidden border border-gray-200">
              <div class="bg-green-600 text-white font-semibold text-xl text-center py-4">
                Project Information
              </div>
              ...
              <div class="px-6 pb-6">
                <dl class="bg-white rounded-lg border border-gray-200 divide-y divide-gray-200">
                  <div v-for="row in infoRows" :key="row.key" class="grid grid-cols-3 sm:grid-cols-4 gap-4 p-4">
                    <dt class="col-span-1 font-semibold text-gray-700">{{ row.label }} :</dt>
                    <dd class="col-span-2 sm:col-span-3 text-gray-700">{{ row.value }}</dd>
                  </div>
                </dl>
              </div>
            </div>
        ...
        <div class="mt-10">
          ...
          <p class="mt-3 text-gray-600 max-w-5xl">{{ results }}</p>
  
          <div class="relative mt-10">
            <!-- horizontal line behind numbers -->
            <div class="absolute left-0 right-0 top-4 h-px bg-gray-200"></div>
  
            <div class="grid md:grid-cols-4 gap-8">
              <div v-for="(f, i) in features" :key="f.title" class="relative">
                <!-- number badge -->
                <div
                  class="absolute -top-3 left-1/2 -translate-x-1/2 w-10 h-10 rounded-full bg-gray-900 text-white
                         text-sm font-bold flex items-center justify-center">
                  {{ (i + 1).toString().padStart(2, '0') }}
                </div>
  
                <div class="flex items-center gap-4">
                  <div class="w-14 h-14 rounded-md bg-green-600 flex items-center justify-center">
        ...
        <div class="mt-14">
          ...
          <div class="mt-6 grid md:grid-cols-3 gap-8">
            <div
              ...
              :key="card.title"
              class="rounded-xl overflow-hidden border border-gray-200 bg-white group"
            >
              <div class="relative">
                <img :src="card.image" :alt="card.title" class="w-full h-56 object-cover" />
                <div class="absolute inset-0 bg-black/0 group-hover:bg-black/25 transition-colors"></div>
                <div class="absolute left-4 bottom-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                  <button class="bg-green-600 text-white px-3 py-2 rounded-md text-sm">View</button>
                  <button class="bg-green-700 text-white px-3 py-2 rounded-md text-sm">Details</button>
                </div>
              </div>
              <div class="p-5">
                <div class="text-lg font-semibold text-gray-900">{{ card.title }}</div>
                <p class="mt-2 text-gray-600 line-clamp-2">{{ card.excerpt }}</p>
              </div>
            </div>
          </div>
        </div>
...
const props = defineProps({
  ...
  intro: {
    type: String,
    default: 'A scelerisque purus semper eget. Placerat in egestas erat imperdiet. Sit facilisis magna.'
  },
  ...
  related: {
    type: Array,
    default: () => [
      { title: 'Silk Fabric',      image: 'silc.jpg',       excerpt: 'A scelerisque purus semper eget. Placerat in egestas erat imperdiet. Sit facilisis magna.' },
      { title: 'Linen Fabric',     image: 'related2.jpg',   excerpt: 'Varius amet mattis vulputate enim nulla. Viverra maecenas accumsan facilisis.' },
      { title: 'Georgette Fabric', image: 'related3.jpg',   excerpt: 'Varius sit amet mattis vulputate enim nulla. Viverra maecenas lacus vel...' }
    ]
  }
})
